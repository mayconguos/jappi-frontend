...
export default function AllShipmentsPage() {
  ...
  useEffect(() => {
    const fetchShipments = async () => {
      ...
      // If user is Admin (role 1) or Coordination (role 5), fetch all shipments
      // Otherwise, fetch just for their company
      const isAdmin = user.id_role === 1 || user.id_role === 5;
      const endpoint = isAdmin ? '/shipping' : `/shipping/${user.id_company || user.id}`;
