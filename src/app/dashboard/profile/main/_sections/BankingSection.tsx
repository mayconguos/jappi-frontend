    ...
    <div className="p-6 md:p-8 space-y-12 animate-in fade-in duration-500">
      ...
      <section>
        ...
        {bankAccounts.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 px-4 bg-gray-50 border-2 border-dashed border-gray-200 rounded-3xl group hover:border-emerald-600/30 transition-colors">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {bankAccounts.map((account, index) => (
              <div key={index} className="group relative p-6 bg-white border border-gray-100 rounded-2xl hover:border-emerald-600/30 hover:shadow-xl hover:shadow-gray-200/40 transition-all duration-300 overflow-hidden">
                {/* Background Decor */}
                <div className="absolute -right-4 -top-4 w-24 h-24 bg-gray-50 rounded-full opacity-50 group-hover:scale-110 transition-transform duration-500" />
                ...
                <div className="flex flex-col h-full relative z-10">
                  <div className="flex items-start justify-between mb-6">
                    ...
                    <div className="flex items-center gap-1">
                      <Button
                        ...
                        variant="ghost"
                        size="icon"
                        className="w-8 h-8 rounded-full bg-gray-50 hover:bg-gray-100 text-gray-600 transition-colors"
                        title="Editar cuenta"
                      >
                        <Edit size={14} />
                      </Button>
                  ...
                  <div className="space-y-4">
                    <div>
                      <p className="text-xs font-semibold text-gray-400 uppercase tracking-widest mb-1">Número de Cuenta</p>
                      <p className="text-xl font-bold text-gray-900 font-mono tracking-tight group-hover:text-emerald-600 transition-colors">
                    ...
                    <div className="flex items-center justify-between pt-4 border-t border-gray-50">
                      ...
                      <div className="text-right">
                        <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-0.5">Titular</p>
                        <p className="text-sm font-semibold text-gray-700 truncate max-w-[150px]">{account.account_holder}</p>
                      </div>
                    </div>

                    {account.cci_number && (
                      <div className="bg-gray-50/80 rounded-xl p-3 flex items-center justify-between">
                        <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">CCI</span>
                        <span className="text-xs font-mono font-semibold text-gray-600">{account.cci_number}</span>
                      </div>
                    )}
                  </div>
                </div>
