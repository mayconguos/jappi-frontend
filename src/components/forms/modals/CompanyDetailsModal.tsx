  ...
  const renderContent = () => {
    if (loading) {
      return (
        <div className="flex items-center justify-center py-12">
          <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-[#0D8F75]"></div>
        </div>
      );
    }
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="bg-emerald-50/50 rounded-xl p-6 border border-emerald-100 flex items-start gap-5">
          <div className="h-14 w-14 rounded-full bg-white flex items-center justify-center text-[#0D8F75] shadow-sm border border-emerald-100 shrink-0">
            <Building2 size={28} />
          </div>
          <div>
            <h2 className="text-xl font-bold text-gray-900 capitalize tracking-tight">
              {data.company.company_name.toLowerCase()}
            </h2>
            <div className="flex items-center gap-2 mt-2">
              <span className="inline-flex items-center rounded-md bg-white px-2.5 py-1 text-xs font-semibold text-[#0D8F75] shadow-sm ring-1 ring-inset ring-emerald-700/10">
          ...
          <div className="border-t border-gray-100">
            <dl className="divide-y divide-gray-100">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 py-4">
                ...
                <dd className="text-sm text-gray-900 sm:col-span-2 font-medium capitalize">
                  {data.user.first_name.toLowerCase()} {data.user.last_name?.toLowerCase()}
                </dd>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 py-4">
                <dt className="text-sm font-medium text-gray-500">Documento</dt>
                <dd className="text-sm text-gray-900 sm:col-span-2 font-medium">
                  {getDocumentTypeLabel(data.user.document_type)}: {data.user.document_number}
                </dd>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 py-4">
                <dt className="text-sm font-medium text-gray-500">Correo Electrónico</dt>
                <dd className="text-sm text-gray-900 sm:col-span-2 font-medium break-all">
                  {data.user.email}
                </dd>
              </div>
            </dl>
                  ...
                  {data.company.phones.length > 0 ? (
                    <div className="flex flex-wrap gap-2">
                      {data.company.phones.map((phone) => (
                        <span key={phone} className="inline-flex items-center px-2.5 py-0.5 rounded-md text-xs font-medium bg-gray-100 text-gray-700 ring-1 ring-inset ring-gray-500/10">
                          {phone}
                        </span>
                      ))}
                    </div>
                  ...
                  <div key={app.id} className="grid grid-cols-1 sm:grid-cols-3 gap-4 py-4 items-center">
                    ...
                    <dd className="text-sm text-gray-900 sm:col-span-2 font-medium">
                      <div className="flex items-center gap-3">
                        <div className="h-8 w-8 rounded-md bg-emerald-50 flex items-center justify-center text-[#0D8F75] font-bold text-[10px] uppercase shrink-0 border border-emerald-100">
